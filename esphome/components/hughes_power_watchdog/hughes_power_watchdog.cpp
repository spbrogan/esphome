...
#define IsFirstHalfOfMessage(data) ( \
  (data[0] == 1) && (data[1] == 3) && (data[2] == 0x20) \
)
...
void HughesPowerWatchdog::dump_config() {
  ...
  if (this->supported_input_lines_ == 1) {
    LOG_SENSOR("  ", "Voltage", this->voltage_l1_);
    LOG_SENSOR("  ", "Current", this->current_l1_);
    LOG_SENSOR("  ", "Power", this->power_l1_);
  } else {
  ...
  switch (event) {
    ...
    case ESP_GATTC_NOTIFY_EVT: {
      if (param->notify.conn_id != this->parent()->conn_id || param->notify.handle != this->handle)
        break;
      ESP_LOGV(TAG, "ESP_GATTC_NOTIFY_EVT: handle=0x%x, data length=%d",
...
void HughesPowerWatchdog::process_tx_notification(uint8_t *value, uint16_t value_len)
{
  ...
  **/
  if (IsFirstHalfOfMessage(value)) {
    ...
    memset(&this->msg_buffer_[20], 0xFF, 20);
    return;
  } 
  ...
  uint8_t line = 0;
  if ( (this->msg_buffer_[37] == 0) && (this->msg_buffer_[38] == 0) && (this->msg_buffer_[39] == 0)) {
    line = 1;
  } else if ( (this->msg_buffer_[37] == 1) && (this->msg_buffer_[38] == 1) && (this->msg_buffer_[39] == 1)) {
    line = 2;
  } else {
    ESP_LOGE(TAG, "process_tx_notification - unsupported line value (0x%X 0x%X 0x%X)", this->msg_buffer_[37], this->msg_buffer_[38], this->msg_buffer_[39]);
    memset(this->msg_buffer_, 0xFF, 40);
    return;
  }
...
}  // namespace hughes power watchdog
}  // namespace esphome
